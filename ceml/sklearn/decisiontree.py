...
class DecisionTreeCounterfactual(SklearnCounterfactual, PlausibleCounterfactualOfDecisionTree):
    ...
    def rebuild_model(self, model):
        ...
        """
        if not isinstance(model, sklearn.tree.DecisionTreeClassifier) and not isinstance(
            model, sklearn.tree.DecisionTreeRegressor
        ):
            raise TypeError(
                f"model has to be an instance of 'sklearn.tree.DecisionTreeClassifier' or 'sklearn.tree.DecisionTreeRegressor' not {type(model)}"
            )

        return  # We do not need to rebuild the model!
    ...
    def compute_all_counterfactuals(self, x, y_target, features_whitelist=None, regularization="l1"):
        ...
        if is_classifier is True:
            ...
            if np.any(
                y_interal != self.model.classes_
            ):  # Handle potential inconsistencies between internal and external representation of class labels
        ...
        if features_whitelist is not None:
            ...
            if len(counterfactuals) == 0:
                raise ValueError(
                    "After filtering for whitelisted features, the tree has no longer a path/leaf yielding the requested outcome specified in 'y_target'."
                )

        return counterfactuals

    def compute_counterfactual(
        self, x, y_target, features_whitelist=None, regularization="l1", C=None, optimizer=None, return_as_dict=True
    ):
...
def decisiontree_generate_counterfactual(
    model, x, y_target, features_whitelist=None, regularization="l1", return_as_dict=True, done=None, plausibility=None
):
    """Computes a counterfactual of a given input `x`.
