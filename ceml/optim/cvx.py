...
def cvx_desc_to_solver(solver_desc):
    if solver_desc == "SCS":
        return cp.SCS
    if solver_desc == "MOSEK":
        return cp.MOSEK
    if solver_desc == "CVXOPT":
        return cp.CVXOPT
    if solver_desc == "OSQP":
        return cp.OSQP
    if solver_desc == "ECOS":
        return cp.ECOS
    if solver_desc == "CPLEX":
        return cp.CPLEX
    if solver_desc == "CBC":
        return cp.CBC
    if solver_desc == "NAG":
    ...
    if solver_desc == "GLPK":
        return cp.GLPK
    if solver_desc == "GLPK_MI":
        return cp.GLPK_MI
    if solver_desc == "GUROBI":
        return cp.GUROBI
    if solver_desc == "SCIP":
        return cp.SCIP
    if solver_desc == "XPRESS":
        return cp.XPRESS
...
class ConvexQuadraticProgram(ABC, SupportAffinePreprocessing):
    ...
    def build_solve_opt(self, x_orig, y, features_whitelist=None, mad=None, optimizer_args=None):
        ...
        if mad is not None:
            ...
            constraints += [Upsilon @ (x - x_orig) <= beta, (-1.0 * Upsilon) @ (x - x_orig) <= beta, I @ beta >= z]
        else:
...
class SDP(ABC):
    ...
    def build_solve_opt(self, x_orig, y, features_whitelist=None, optimizer_args=None):
        ...
        # Build the final program
        f = cp.Minimize(cp.trace(I @ X) - 2.0 * x.T @ x_orig)
        prob = cp.Problem(f, constraints)
...
class DCQP(SupportAffinePreprocessing):
    ...
    def build_program(
        ...
        self.pccp = PenaltyConvexConcaveProcedure(
            model, Q0, Q1, q, c, A0_i, A1_i, b_i, r_i, features_whitelist, mad, optimizer_args
        )
...
class PenaltyConvexConcaveProcedure(SupportAffinePreprocessing):
    ...
    def __init__(
        self, model, Q0, Q1, q, c, A0_i, A1_i, b_i, r_i, features_whitelist=None, mad=None, optimizer_args=None, **kwds
    ):
        self.model = model
        self.mad = mad
    ...
    def solve_aux(self, xcf, tao, x_orig):
        try:
            ...
            for i in range(len(self.A0s)):
                ...
                c = (
                    self.rs[i]
                    + np.dot(xcf, np.dot(xcf, self.A1s[i]))
                    - 2.0 * var_x_prime.T @ np.dot(xcf, self.A1s[i])
                    - s[i]
                )
            ...
            if self.mad is not None:  # TODO: Right now, mad != 1 is not supported.
                ...
                f = cp.Minimize(
                    cp.quad_form(var_x_prime, self.Q0)
                    + self.q.T @ var_x_prime
                    + self.c
                    + np.dot(xcf, np.dot(xcf, self.Q1))
                    - 2.0 * var_x_prime.T @ np.dot(xcf, self.Q1)
                    + s.T @ (tao * s_c)
                )
            constraints += [s >= s_z]
    ...
    def compute_counterfactual(self, x_orig, y_target, x0):
        ...
        while cur_tao < self.tao_max:
            cur_xcf = xcf
            if (
                cur_xcf.shape == x_orig.shape
            ):  # Apply transformation is necessary - xcf is computed in the original space which can be different from the space the model works on!
...
class HighDensityEllipsoids:
    def __init__(
        self, X, X_densities, cluster_probs, means, covariances, density_threshold=None, optimizer_args=None, **kwds
    ):
        self.X = X
...
class PlausibleCounterfactualOfHyperplaneClassifier:
    ...
    def _build_constraints_plausibility_opt(self, var_x, y):
        ...
        if self.hyperplane_w.shape[0] > 1:
            for i in range(self.hyperplane_w.shape[0]):
                if i != y:
                    constraints += [
                        ...
                        @ (self.hyperplane_w[i, :] - self.hyperplane_w[y, :])
                        + (self.hyperplane_b[i] - self.hyperplane_b[y])
                        + self.epsilon
                        <= 0
                    ]
        else:
            if y == 0:
                return [
                    (self.projection_matrix @ (var_x - self.projection_mean_sub)).T @ self.hyperplane_w.reshape(-1, 1)
                    + self.hyperplane_b
                    + self.epsilon
                    <= 0
                ]
    ...
    def build_solve_plausibility_opt(self, x_orig, y, mad=None):
        ...
        if self.density_constraint is True:
            ...
            constraints += [
                cp.quad_form(self.projection_matrix @ (x - self.projection_mean_sub) - x_i, cov) - self.ellipsoids_r[i]
                <= 0
