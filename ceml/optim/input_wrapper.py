...
class InputWrapper:
    ...
    def __init__(self, features_whitelist, x_orig, **kwds):
        ...
        if self.features_whitelist == []:
            raise ValueError(
                "'features_whitelist' does not contain any features. If you do not want to restrict the features, use 'features_whitelist=None' instead of '[]'"
            )
    ...
    def complete(self, x):
        ...
        return np.array(
            [
                x[self.features_whitelist.index(i)] if i in self.features_whitelist else self.x_orig[i]
                for i in range(self.x_orig.shape[0])
            ]
        )
